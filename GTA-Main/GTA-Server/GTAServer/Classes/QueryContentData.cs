...
namespace GTAServer
{
    public class QueryContentData
    {
        ...
        public class Mission
        {
            ...
            public JToken da { get; set; }  // <-- flexible: object or string
        ...
        public static string GenerateXml(string[] contentids)
        {
            ...
            if (root?.r == null)
                root = new Root { r = new List<Record>() };

            int Total = root.r.Count(record =>
                Array.Exists(contentids, id => id == record._c || (record.m != null && id == record.m._rci))
            );
            ...
            {
                if (!Array.Exists(contentids, id => id == record._c || (record.m != null && id == record.m._rci)))
                    continue;

                var xml = new XElement(ns + "r",
                    new XAttribute("c", record._c ?? string.Empty)
                );
                ...
                if (record.m != null)
                {
                    ...
                    // Handle 'da' as string or object
                    if (record.m.da != null)
                    {
                        string daStr;
                        if (record.m.da.Type == JTokenType.String)
                            daStr = record.m.da.ToString();
                        else
                            daStr = record.m.da.ToString(Newtonsoft.Json.Formatting.None); // object -> compact JSON string

                        mElement.Add(new XElement(ns + "da", new XCData(daStr)));
                    }
