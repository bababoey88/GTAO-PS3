...
namespace GTAServer
{
    public class MultipartFormData
    {
        ...
        public List<byte[]> GetTrackIds(byte[] data)
        {
            List<byte[]> trackIds = new List<byte[]>();
            ...
            {
                // Look for something like: trackId=1234 or TRACKID=1234
                if (line.Contains("trackId=") || line.Contains("TRACKID="))
                {
                    int idx = line.IndexOf('=');
                    if (idx != -1 && idx + 1 < line.Length)
                    {
                        string idStr = line.Substring(idx + 1).Trim();
                        if (!string.IsNullOrEmpty(idStr))
                        {
                            trackIds.Add(Encoding.UTF8.GetBytes(idStr));
                        }
                    }
                }
            }
